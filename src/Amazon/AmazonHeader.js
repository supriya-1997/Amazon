import React ,{useContext} from 'react';

import { BrowserRouter as Router, Link, Route } from 'react-router-dom';
import Cart from '../components/Cart'

import {CartContext } from '../CartContext';
import Mobiles from '../components/Mobiles'
import CheckOut from '../components/CheckOut';
import Dress from '../components/Dress'
import DeliveryLocation from './DeliveryLocation'
const AmazonHeader = () => {
  const {mobiles,dress} = useContext(CartContext);
  const {state :{cart}}=useContext(CartContext);
    return (
        <>
        
            <div className=" row bg-dark fixed-top " >
            <div className="collapse" id="navbarToggleExternalContent">
    <div className="bg-dark p-4">
      
      <span className="text-muted">SHOP BY CATEGORY</span>
      <br/>
      <Link to="/" className="text-white">Amazon Music</Link> <br/>
      <Link to="/" className="text-white">Echo & Alexa</Link> <br/>
      <Link to="/" className="text-white">Fire Tablets</Link> <br/>
      <Link  to="/" className="text-white">Fire TV</Link> <br/>
      <Link  to="/" className="text-white">Kindle E-readers & Books</Link> <br/>
      <Link  to="/" className="text-white">Appstore for Android</Link> <br/>
      <Link  to="/" className="text-white">Subscribe & Save</Link> <br/>
      <Link  to="/" className="text-white">Smart Home</Link> <br/>
      <Link  to="/" className="text-white">Amazon Physical Stores</Link> <br/>
      <Link  to="/" className="text-white">Gift Cards</Link> <br/>
     <Link  to="/" className="text-white">#FoundItOnAmazon</Link> <br/>
      <Link  to="/" className="text-white">Amazon Subscription Boxes</Link> <br/>
      <Link  to="/" className="text-white">Amazon Live</Link>
    </div>
  </div>
  <nav className="navbar navbar-dark bg-dark">
    <button className="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarToggleExternalContent" aria-controls="navbarToggleExternalContent" aria-expanded="false" aria-label="Toggle navigation">
      <span className="navbar-toggler-icon"></span>
    </button>
  </nav>
            <div className="col-sm-3 ml-5"> 
            
                  <Link to="/home"> <img src="data:image/png;base64," width="80" height="70"/></Link></div>
                <div className="col-sm-5 mt-3">
                    <form action="" method="get">
                    <div className="input-group mb-3">
  <div className="input-group-prepend">
    <span className="input-group-text btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">All</span>
    
    <div className="dropdown-menu">
      <Link to="/" style={{color:'black'}}>All Departments</Link><br/>
      <Link to="/" style={{color:'black'}}>Arts & Crafts</Link><br/>
      <Link to="/" style={{color:'black'}}>Automotive</Link><br/>
      <Link to="/" style={{color:'black'}}>Automotive</Link><br/>
      <Link to="/" style={{color:'black'}}>Baby</Link><br/>
      <Link to="/" style={{color:'black'}}>Beauty & Personal Care</Link><br/>
      <Link to="/" style={{color:'black'}}>Books</Link><br/>
      <Link to="/" style={{color:'black'}}>Computers</Link><br/>
      <Link to="/" style={{color:'black'}}>Digital Music</Link><br/>
      <Link to="/" style={{color:'black'}}>Electronics</Link><br/>
      <Link to="/" style={{color:'black'}}>Kindle Store</Link><br/>
      <Link to="/" style={{color:'black'}}>Prime Video</Link><br/>
     < Link to="/" style={{color:'black'}}>Women's Fashion</Link><br/>
      <Link to="/" style={{color:'black'}}>Men's Fashion</Link><br/>
      <Link to="/" style={{color:'black'}}>Girl's Fashion</Link><br/>
      <Link to="/" style={{color:'black'}}>Boy's Fashion</Link><br/>
      <Link to="/" style={{color:'black'}}>Deals</Link><br/>
      <Link to="/" style={{color:'black'}}>Health & Household</Link><br/>
      <Link to="/" style={{color:'black'}}>Home & Kitchen</Link><br/>
      <Link to="/" style={{color:'black'}}>Industrial & Scientific</Link><br/>
      <Link to="/" style={{color:'black'}}>Luggage</Link><br/>
      <Link to="/" style={{color:'black'}}>Movies & TV</Link><br/>
      <Link to="/" style={{color:'black'}}>Music,CD & Vinyl</Link><br/>
      <Link to="/" style={{color:'black'}}>Pet Supplies</Link><br/>
      <Link to="/" style={{color:'black'}}>Software</Link><br/>
      <Link to="/" style={{color:'black'}}>Sports & Outdoors</Link><br/>
      <Link to="/" style={{color:'black'}}>Tools & Home Improvement</Link><br/>
      <Link to="/" style={{color:'black'}}>Toys & Games</Link><br/>
      <Link to="/" style={{color:'black'}}>Video Games</Link>
  </div>
  </div>
  <input type="text" className="form-control" />
  <div className="input-group-append">
  <button className="bg-warning" type="submit">
                                <i className="fa fa-search" aria-hidden="true"></i>
                            </button>
  </div>
</div>
                        
                    </form>
                </div>
                <div className="dropdown">
                   <a className="btn btn-dark dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        Hello,Sign in<br />
   Accounts & Lists
    </a>
    
                    <div className="dropdown-menu" aria-labelledby="dropdownMenuLink">
                        <div className="row" style={{width:"600px"}}>
                            <div className="col-xl-6">
                            <a className="dropdown-item" href="#">Your Account</a>
                        <a className="dropdown-item" href="#">Your Orders</a>
                        <a className="dropdown-item" href="#">Buy Again</a>
                        <hr/>
                        <a  className="dropdown-item" href="#">Your Recommendations</a>
                        <a  className="dropdown-item" href="#">Your Subscribe & Save Items</a>
                        <a  className="dropdown-item" href="#">Memberships & Subscriptions</a>
                        <a  className="dropdown-item" href="#">Your Service requests</a>
                        <a  className="dropdown-item" href="#">Your Garage</a>
                        <a  className="dropdown-item" href="#">Your Fanshop</a>
                        <a  className="dropdown-item" href="#">Your Pets</a>
                        <a  className="dropdown-item" href="#">Your Content & devices</a>
                        <a  className="dropdown-item" href="#">Your Music Library</a>
                        
                        <a  className="dropdown-item" href="#">Your Amazon Drive</a>
                        <a  className="dropdown-item" href="#">Your Prime Video</a>
                        <a  className="dropdown-item" href="#">Your Kindle Unlimited</a>
                        <a  className="dropdown-item" href="#">Your Watchlists</a>
                        <a  className="dropdown-item" href="#">Your Video purchases & rentals</a>
                        <a  className="dropdown-item" href="#">Your Android Apps & devices</a>
                        <br/>
                        <br/>
                        <hr/>
                        <a  className="dropdown-item" href="#">Help</a>
                            </div>
                           
                           
                            <div className="col-xl-6">
                            <a className="dropdown-item" href="#">Your Lists</a>
                        <a className="dropdown-item" href="#">Create a List</a>
                        <a className="dropdown-item" href="#">Find a List or Registry</a>
                       
                        <a  className="dropdown-item" href="#">Find a Gift</a>
                        <a  className="dropdown-item" href="#">Save Items from the Web</a>
                        <a  className="dropdown-item" href="#">Wedding registry</a>
                        <a  className="dropdown-item" href="#">baby registry</a>
                        <a  className="dropdown-item" href="#">Friends & Family Gifting</a>
                        <a  className="dropdown-item" href="#">AmazonSmile Charity Lists</a>
                        <a  className="dropdown-item" href="#">Pantry Lists</a>
                        <a  className="dropdown-item" href="#">Your Hearts</a>
                        <a  className="dropdown-item" href="#">Explore Idea Lists</a>
                        <a  className="dropdown-item" href="#">Explore Showroom</a>
                        <a  className="dropdown-item" href="#">Take the Home Style Quiz</a>
                        <br/>
                        <br/>
                        <br/>
                        <br/>
                        <br/>
                        <br/>
                        <br/>
                        <br/>
                        <br/>
                        <hr/>
                        <a  className="dropdown-item" href="#">New Customer?Start Here</a> 
                        
                       <Link to="/Login"> <a  className="dropdown-item" href="#">Sign in</a></Link>
                            </div>
                            
                            </div>
                       
                    </div>
                </div>
                <div className="col text-light  mt-3">Returns <br/>& Orders</div>
                <div className="col text-light  mt-3"><i className="fa fa-shopping-cart" ></i><Link to="/cart"><span className="text-light">Cart({cart.length})</span></Link></div>
            </div>
            
            <div className="shadow p-2 mb-4 bg-light rounded">
                <div className=" row mt-5 py-4">
                <div className="col-sm-1">
                  <div className="row">
                    <div className="col-lg-6">
                    <i className="material-icons" style={{fontSize:'36px'}}>location_on </i> 
                    </div>
                    <div className="col-lg-6 text-dark" >
                   
                    <Link to="/delivery"><b className="text-dark">Delivery to India</b></Link>
                  
                    </div>
                    </div>
                   </div>
                   &nbsp;&nbsp;&nbsp;&nbsp;
                    {/*<div class="col-sm-2">  <Link to="/mobile"><b class="text-dark">Mobiles</b></Link></div>*/}
                    <div className="col-sm-1"><Link to="/mobile" onClick={()=>mobiles()}><b className="text-dark">Mobiles</b></Link></div>
                    <div className="col-sm-1"><Link to="/cloth" onClick={()=>dress()}><b className="text-dark">Dress</b></Link></div>
                    
                    <Route path="/delivery" exact strict component={DeliveryLocation}></Route>
                   <Route path="/mobile" exact strict component={Mobiles}></Route>
                   <Route path="/cloth" exact strict component={Dress}></Route>
                    <Route path="/checkout" exact  strict component={CheckOut}></Route>
           <Route path="/cart" exact strict component ={Cart}></Route>
           
                </div>
            </div>
           

          
           
        </>

    )
}

export default AmazonHeader